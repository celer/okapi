  ...
  if(typeof thing=="function"){
    ...
    for(var i in thing){
      if(this.dao.columns[i]){
        if(typeof thing[i]=="undefined" || typeof thing[i]=="null"){
          this.isNull(i);
        } else {
          this.eq(i,thing[i]);
        }
...
DAO.IDRef.fromSQLValue=function(input){
  if(input==null) return null;
...
DAO.Object.prototype.column=function(name,def){
  ...
  var modifiers = ["pk","notNull","default","unique","type","autoKey","ref","values","length"];
...
//Calculate the creation dependencies
DAO._calcDeps=function(){
  var tables = Array.prototype.slice.call(arguments);

  var daos = {};
  var deps = {};

  tables.forEach(function(dao){
    daos[dao.table]=dao;
    deps[dao.table]=[];
    for(column in dao.columns){
      var c = dao.columns[column];
      if(c.ref && c.ref.dao){
        deps[dao.table].push(c.ref.dao.table);
      }
    } 
  });
  ...
  //create all tables with no deps
  for(var d in deps){
    if(deps[d].length==0){
      tasks.push(daos[d]); 
      created[d]=true;
  ...
  var toBeCreated=Object.keys(deps).length;
  while(Object.keys(deps).length>0){
    ...
    if(toBeCreated==Object.keys(deps).length)
      throw new Error("Cannot resolve table dependencies");
  }
...
DAO.dropTables=function(){
  ...
  tasks = DAO._calcDeps.apply(null,tables); 

  tasks.reverse();

  async.mapSeries(tasks,function(dao,done){
