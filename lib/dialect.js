...
DAO.SQLDialect.prototype.upsert=function(updater,onComplete){
  ...
  if(!this.sql.upsert){ 
    ...
    updater.dao.update(updater.values).onComplete(function(err,res){
      if(err) return onComplete(err);
      if(res && res.changedRows==0){
        updater.dao.insert(updater.values).onComplete(function(err,res){
            if(err) return onComplete(err);
            if(res) {
    ...
    for(var i in updater.values){
      data[i]=updater.values[i];
    }


    var sql = this.buildUpsert(updater,data);
...
DAO.SQLDialect.prototype.buildSelectResults=function(finder,res,tableAliases){
  if(finder._join && res){
    ...
    res = res.map(function(r){ 
      return finder.dao.fromSQLValue(r);
    });
  }
  return res;
}
...
DAO.SQLDialect.prototype.buildSelect=function(finder,data,tableAliases){
    ...
    context.columns=function(tableName, as){   
        ...
        if(tableName){
          ...
          if(joins.length>0){
            joins.map(function(join){
              for(var table in join.columns()){
                jc.push(context.columns(context.tables[table].tableName(),table));
              }
            });
            return jc.join(", ");
          } else {
            return context.columns(finder.dao.tableName());
          }
        }
    }
...
DAO.SQLDialect.prototype.doDelete=function(sql,data,deleter,onComplete){
  this.query(sql,data,function(err,res){
    if(err) return onComplete(err);
...
DAO.SQLDialect.prototype.delete=function(deleter,onComplete){
  ...
  this.doDelete(psql.sql,psql.data,deleter,onComplete);
}
