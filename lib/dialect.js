...
DAO.SQLDialect.prototype.buildUpdateResults=function(updater,err,res,onComplete){
...
DAO.SQLDialect.prototype.update=function(updater,onComplete){
	...
	self.query(sql,data,function(err,res){
		self.buildUpdateResults(updater,err,res,onComplete);
	},updater.dao,"update");
...
DAO.SQLDialect.prototype.find=function(finder,onComplete){
	...
	self.query(sql,data,function(err,res){
		...
		if(res){
			...
			res = self.buildSelectResults(finder,rows,tableAliases);
		}
		return onComplete(err,res);
	},finder.dao,"select");
}
...
DAO.SQLDialect.prototype.insert=function(inserter,onComplete){
	...
	this.query(sql,data,function(err,res){
		self.buildInsertResult(inserter,err,res,onComplete);
	},inserter.dao);
}
